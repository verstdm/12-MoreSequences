...
def run_test_generate_points_on_circle():
    ...
    # ------------------------------------------------------------------
    expected = [rg.Point(220, 30),  # All numbers are approximate.
                rg.Point(200, 50),
                rg.Point(180, 30),
                rg.Point(200, 10)]

    circle = rg.Circle(rg.Point(200, 30), 20)
...
def run_test_draw_points_on_circle():
    ...
    # ------------------------------------------------------------------
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  8 purple dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(100, 150), 100)
    draw_points_on_circle(window, circle, 6, 'purple')
    window.close_on_mouse_click()
...
def run_test_polygon():
    ...
    # ------------------------------------------------------------------
    title = 'POLYGON tests 3:  5 segments with thin black lines.'
    window = rg.RoseWindow(650, 450, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 3
    polygon(window, circle, 5, 'black', 2)
    window.close_on_mouse_click()
...
def run_test_fancy_polygon():
    ...
    # ------------------------------------------------------------------
    title = 'FANCY POLYGON test 4:  40 red lines on black circle, hops = 19.'
    window = rg.RoseWindow(500, 4000, title)

    circle = rg.Circle(rg.Point(250, 200), 200)
    circle.fill_color = 'black'
    fancy_polygon(window, circle, 40, 19, 'red', 3)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    for i in range(len(points)):
        if len(points) > i + hops_to_next_point:
